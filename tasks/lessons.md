...
## 2026-07-26: 「這個改動會拆掉 X」在下結論前必須把 X 的定義式算一遍
- Context：spec v1 主張 `risk_monitor` 的「雙側各減 0.08」會拆掉對沖，理由是「兩側都減」。reviewer 指出：`delta = long − short`，兩側等量減 ⇒ **delta 完全不變**，它降的是 gross 不是中性度。我的整段理由算術上就錯，而且據此提出的修法（只減大側）在 `long == short`（正是這份 spec 追求的終態）會輸出零訂單 = 移除唯一的 de-gross 煞車。
- Error：我在同一份 spec 裡才剛用 `long − short` 精算過主線機制（進0.02/出0.04 的不對稱），換到另一個模組就改用**語言直覺**判斷（「兩側都減」聽起來就像拆對沖）。**同一份文件裡，量化嚴謹度不會自動傳染到下一節。**
- 附帶損害：這個錯誤前提被我拿去問使用者並取得裁決 ⇒ 使用者的決定建立在錯的事實上，只能回頭重問。**錯誤前提污染下游決策，代價不是改一行字。**
- Rule：凡是「這個改動會惡化/改善 X」的斷言，X 有定義式就把式子寫出來代數字，沒有定義式就先定義。特別是**當 X 是差值或比率**（delta、ratio、spread、margin usage）時，「兩邊都變大/都變小」對它的影響**經常是零**。送使用者裁決前，把裁決依賴的每個事實斷言標記「已驗算 / 未驗算」，未驗算的不得進選項描述。

## 2026-07-26: 刪掉一個條件之前，先問「有沒有測試是刻意靠它觸發的」
- Context：spec 要刪 `tp_quantity` 的 `or opposite >= threshold` 子條件。`tests/test_backtest_matching.py:147` 的 fixture **刻意**設 `limit_multiplier=100`（讓另一條路徑不可能）+ `threshold_multiplier=1.0`，好讓加倍**只**由這條子條件觸發，用來檢驗 reduce_only clamp。刪掉後 tp qty 從 2.0 變 1.0，clamp 變 no-op，而三條斷言值**恰好完全不變** ⇒ 測試繼續綠、鑑別力歸零。
- Error：我盤點了 caller，沒盤點「哪些測試的**前置條件**依賴這個分支」。fixture 註解裡寫得很清楚（「確保加倍只由 opposite_position 這條路徑觸發」）——我沒讀它就動了它依賴的東西。
- Rule：刪除或收緊任何條件前，`grep` 該條件涉及的每個欄位在 `tests/` 的出現處，**讀 fixture 的註解與刻意設定的極端值**（拉到不可能觸發的 100、壓到剛好等於門檻的 1.0，都是「我在用這條路徑」的訊號）。判準：問「這個測試原本靠什麼觸發？改動後還觸發嗎？若不觸發，斷言值會變嗎？」**斷言值不變 = 靜默失去鑑別力**，比測試變紅危險得多（通則 3）。

## 2026-07-26: 成本網格的「網格」本身要先驗證真值落在裡面
- Context：requote 實驗掃了 fee ∈ {2,4}bps × slip ∈ {0,1,2}bps，做了完整 cost sensitivity，§6.4「排序跨成本翻轉」判 FAIL，據此保留現行語意。實查交易所 `commissionRate`：BNBUSDC maker = **0**、taker 4bps，而引擎 19/19 成交全 maker。
- Error：**真值 0 不在網格內**。cost sensitivity 做得再完整，也只是在「錯誤區間內」做的完整。補跑 fee=0 後 §6.4 由 FAIL 轉 PASS（排序跨 slip 全一致）⇒ 那個 FAIL 完全是錯誤 fee 值造成的。`backtest/config.py:37` 的註解「maker 手續費 0.02%」從未查證，是通則 1 第 4 條（接線斷在 repo 外）的第七次現場。
- 附帶：§6.3 在 fee=0 下**仍 FAIL**（上漲段逆選擇 -16.8）⇒ 結論方向沒變，但「成本吃光 grinding」這個**理由**是錯的。錯的理由會讓下一輪找錯方向（會去想怎麼降成本，而真障礙是逆選擇）。
- Rule：做 sensitivity 之前先問「真值在網格裡嗎」，並用外部系統實查（費率、槓桿、marginMode、最小下單量都屬此類）把真值釘成網格的一個點。**結論對但理由錯，要跟結論錯一樣認真修**——理由是下一輪的路標。促銷性質的外部參數（零費率）另記「非永久」與監控條件。

## 2026-07-26: 「不對稱的量」在雙側策略上會單調摧毀人工建立的結構
- Context：使用者手動補空建立 delta 中性對沖（0.58/0.36）。11 天後空頭 0.36→0.20（-44%），多頭不變。原以為是「網格有動」的隨機結果。
- Error：這是**確定性機制**，不是隨機。`tp_quantity` 在持倉 > `position_limit` 後把止盈量加倍（進 0.02/出 0.04），兩側都適用 ⇒ 每完整往返雙側各淨減 0.02。等量減對**基數小的那側**是成比例更快的傷害（0.20 的側 3 倍快於 0.60）。逐筆對帳零殘差。
...
## 2026-07-30: 單點標量判準比不了軌跡——帶號與絕對值在穿越 0 時會各錯一次
- Context：A6 的 gate 判「新規則的 `final abs(delta)` 每窗 ≤ 舊規則」。實跑 3/8 窗 FAIL。診斷後發現 A/W2 的帶號 delta 逐日**每一天**都低於舊規則，只是末端穿越 0 繼續走到 −0.16，`abs()` 因此反而變大。
- Error：v1 我用帶號值當「收斂」被 spec reviewer 判 BL3（帶號下降 ≠ 收斂），我改用 `abs`；`abs` 在穿越 0 的路徑上又反向誤判。**兩個方向都錯過一次，根因同一個：拿單點標量去描述一條軌跡。**
- 附帶：另外 2 項 `abs` 劣化是真的，但舊規則的「低 delta」是把**兩側都拆到接近 0** 換來的（窗口末 `min(L,S)` 0.08 vs 新規則 0.28）——「淨曝險低」與「對沖有效」是兩件事，低到沒有倉位也叫低。
- Rule：路徑型指標（delta、曝險、庫存）的判準至少要兩項：**峰值**（`max abs`）+ **逐點軌跡不劣化**，終點值只能當補充。且任何「風險變小」的宣稱要配一個**存量**指標（`min(L,S)`、gross），否則「把部位清光」會被判成最安全。
- 事後改判準是回測最經典的自欺 ⇒ 若診斷後確實該改，必須：留在 spec 內（本次 §5.3）、標明是看過結果後才改、寫清修訂前後的判定差異、由使用者核可。**改成更嚴的版本（本次新增逐日軌跡要求）比放寬容易辯護，放寬的那條要單獨給理由。**
