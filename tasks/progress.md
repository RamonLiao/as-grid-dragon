...
## Current Task
#4 回測/實盤策略脫鉤 — **程式碼完成 (Task 1-9 + 8b)，雙輪 review 收斂 (Ship as-is)，未 push**。Subagent-Driven 執行，10 commits 800fd98..7186203，全套 **187 passed**。
...
- SDD ledger 詳情：`.superpowers/sdd/progress.md`。#1-#3 先前完成(86acd3e..800fd98)，全部尚未 push。

## TODO — 架構審查修復清單（2026-07-03，詳見 tasks/notes.md，依序修）
- [x] **#1 (P0) 下單路徑加固** — commit 86acd3e：clientOrderId + 指數 backoff + 斷路器（僅開倉單成功重置，防 TP 交錯失效）+ 封鎖期不白撤 + `position_adjust_cooldown`（預設 5s）；35 新測試，全套 109 passed；dual-review 兩輪收斂 + verifier PASS（尚未 push）
...
- [~] **#4 (P0) 回測/實盤策略脫鉤**：程式碼完成 (Task 1-9+8b, 800fd98..7186203, 187 passed)，雙輪 review Ship as-is，未 push。**剩 Task 10 人工 24h replay zero-diff 上線驗收**。詳見上方 Current Task。
- [ ] **#5 (P1) 回測成本模型**：補滑價 + 資金費率（實盤有 FundingRateManager，回測沒有）
- [~] **#6 (P1) Bandit 狀態持久化**：brainstorming→spec→plan 完成，待實作。spec `docs/superpowers/specs/2026-07-04-bandit-state-persistence-design.md`、plan `docs/superpowers/plans/2026-07-04-bandit-state-persistence.md`（6 tasks TDD）。設計：純層 `grid_engine/bandit_persistence.py`(save/load 原子寫+fsync) + bandit.py 加 `arm_signature` + bot 接線 3 處（run 載入/每 10 筆評估後條件存/stop 收尾）；`grid_engine/config.py` 加 `bandit_state_path`/`bandit_state_max_age_sec`。量化 review 折入：arm_signature 不簽 sizing（reward 已驗 scale-invariant，砍 reward_signature）、只復原學到統計不復原瞬時選擇、非有限值 sanitize、max_age 過期冷啟動、replay-invariant 守門。**注意 live config 是 `grid_engine/config.py` 非 `config/models.py`（舊 core）**
- [ ] **#7 (P1) MaxGridBot god class 拆分**：WS/下單/風控/同步/通知分層（等 #2 #3 落地後再動）
