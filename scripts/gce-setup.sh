...
echo "  2. cd $REPO_DIR"
echo "  3. docker compose -f docker-compose.terminal.yml run --rm as-grid"
echo "  4. 在 TUI 中設定 API Key (選項 7) 和 Telegram (選項 9)"
echo "  5. 到交易所綁定此 VM 的 IP 白名單"
echo ""
echo "常用指令："
echo "  啟動:   docker compose -f docker-compose.terminal.yml run --rm as-grid"
echo "  斷開:   Ctrl+P, Ctrl+Q (container 繼續跑)"
echo "  接回:   docker attach as-grid"
echo "  停止:   docker compose -f docker-compose.terminal.yml down"
echo ""
echo "VM 外部 IP:"
