...
- **A5 是 regression guard，不是 live fix 的證據**：risk_monitor 觸發條件在現行資本下
  **不可達**（雙側同時 ≥0.64 ⇒ gross 1.28 ≈ 730 USDC 名目，所需保證金遠超可用 17.955）。
  它防的是未來入金後的漂移。
- **⚠️ 對沖側零收斂力，gross 的實質上限是保證金耗盡（2026-07-30 外部 review 提出，使用者已裁決接受）**：
  新規則下較小側 entry 1× / TP 1× ⇒ **進出完全對稱、該側沒有任何往下的力**，只隨價格路徑漂移，
  直到它超過另一側才輪到它加倍。被刪掉的 `opposite >= threshold` 正是「對手側達裝死量級時強制抽乾我」
  的機制——移除它是本 spec 的目的，但代價是 gross 少了一道主動收縮力。
  剩下的兩道後備**在現行資本下都不可達**（現價 ~572、5x）：
  | 防線 | 需要的 gross | 需要的保證金 | 現況 |
  |---|---|---|---|
  | risk_monitor 雙向減倉（雙側各 0.64） | 1.28 | ~146 USDC | 權益 113.8 ⇒ **不可達** |
  | 裝死（雙側各 0.80） | 1.60 | ~183 USDC | **不可達** |
  | 保證金耗盡 / 交易所 `-2019` 拒單 | ~1.0 | = 全部權益 | **這才是實際的煞車** |
  ⇒ 與健檢 §8「mult=40 在現行資本下是惰性參數」一致。**回測完全沒建模 `-2019` 拒單通道**，
  所以任何「gross 不會失控」的信心來自交易所而非本 repo。
  **使用者裁決（2026-07-30）：接受此狀態，讓現有倉位順著新規則慢慢收斂到 hedge。**
  ⚠️ 這個裁決綁定「不入金」——**若日後入金使權益接近 146 USDC，risk_monitor 會從不可達變成活的
  生產程式碼**，屆時 A5 的 regression guard 身分失效，須重新評估它的觸發門檻與減倉量。
...
- **⚠️ W1 與 W3 的交易次數低於統計門檻（2026-07-30 Round 2 專案規則輪發現）**：
  專案 quant rules 要求「交易次數 < 30 的回測一律標註樣本不足，統計上不可信」。實測 `round_trips`——
  **W1 = 28**（兩規則同）、**W3 = 15**（兩規則同）、W2 = 48~50、full = 84~95。
  ⇒ **W1 與 W3 樣本不足**，而 §5.3 裡 gate (iii) 失敗的兩格（A/W1 −7.221、B/W1 −2.906）
  與 gate (i) 失敗的一格（B/W3）**正好全部落在這兩個窗口**。
  這對修訂後的判定是**雙向**的：既不能拿 W1 的 equity 劣化當「新規則有害」的證據，
  也不能拿 W3 的改善當「新規則有利」的證據。**修訂後 A6 的實質證據力集中在 W2（48~50 筆）
  與 full（84~95 筆）**——這兩個窗口新規則的 equity 與 maxDD 皆改善，結論不因此翻轉，
  但「8/8 全窗改善」這句話的份量要打折：其中 4 格（W1×2、W3×2）樣本不足。
  ⚠️ 即使 ≥30 也只是最低門檻（必要非充分），可信度仍取決於獨立事件數與 Sharpe 標準誤。
