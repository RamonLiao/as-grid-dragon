...
| `OrderExecutor` | `place_order`(370)、`_register_order_failure`(409)、`cancel_orders_for_side`(432)、`_close_symbol_positions`(342)、cooldown 查詢介面 | `_order_fail_counts`、`_order_block_until`、`_order_seq`、`last_order_times` | gateway、exchange、state、notifier、config、precisions、SymbolLocks |
| `SyncService` | `sync_all`(175)、`_sync_positions`(196)、`_sync_orders`(224)、`_sync_account`(256)、`_sync_funding_rates`(184) | `_sync_lock`、`last_sync_time` | gateway、exchange、state、SymbolLocks、funding_manager |
| `WsClient` | `_websocket_loop`(938)、`_get_listen_key`(171)、`_keep_alive_loop`(980) | `listen_key` | gateway、exchange、config、handler callbacks（bot 註冊） |
...
| `DailyReporter` | `_daily_pnl_loop`(992) | 排程時間狀態 | state、notifier、config |
...
- 網格鏈：`adjust_grid`、`_should_adjust_grid`、`_grid_cooldown_passed`、`_grid_step`、`_place_grid`、`_execute_side_decision`、`_build_bundle`、`_build_inputs`、`_log_decision`、`_get_adjusted_quantity`。
...
- **新增組件級測試（少量）**：
  ...
  - OrderExecutor：斷路器狀態機在新類上的 smoke（既有 35 測遷移後已覆蓋主體）。
...
1. `locks.py` SymbolLocks + `rest_gateway.py` RestGateway（無依賴的葉子先行）
2. `sync_service.py` SyncService
3. `order_executor.py` OrderExecutor
4. `ws_client.py` WsClient（handlers 留 bot，callback 註冊）
5. `risk_monitor.py` RiskMonitor + `reporting.py` DailyReporter
6. bot.py 收尾：`__init__` 組裝、殘留引用清理、行數確認
7. 全套回歸 + monkey + 補組件級測試

每步結束全套測試必須綠才 commit（紅的中間態不落 commit）。
