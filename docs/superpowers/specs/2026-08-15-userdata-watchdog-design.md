...
3. **活體驗收**（重啟後）：
   ...
   - ~~**引擎啟動後 75 分鐘**（= attempt 1 於 t0+600，再經 300/900/2700 退避）進入 `given_up`~~
     ⚠️ **2026-08-16 活體驗收更正：75 分鐘是錯的，且不存在正確的固定分鐘數。**
     實測 **t0+118 分鐘**才進 `given_up`（09:29:08 啟動 → 11:27:08 終態），見 §8.2 的實測時間表。
     正確的判準是：進入 `given_up` 前**發出第二封 Telegram**，且**不再**出現重連 log，
     整個過程 Telegram 恰好 **2 封**、強制重連恰好 **3 次**——**次數可判定，時間不可判定**。
   ...
   （⚠️ **2026-08-16 再更正：75 分鐘同樣低估，實測 118 分鐘。原因是每階要等下一次
   requote 補證據，不是純退避累加——見 §8.2 第 4 項。**）
...
4. **`given_up` 的抵達時間是 requote 事件的函數，不是時間的函數（2026-08-16 活體實測）。**
   §5.2 讀起來像「退避累加 300+900+2700 = 65 分，加第一次判死的 600s = 75 分後進終態」。
   **實際不是。** 每一階的耗時是：

   ```
   max(退避, 600s 靜默) + 等下一次 requote 把 orders_since_event 補回 K
   ```

   因為 §8.2-1 的「重連後要重新取證」設計，`record_event()` 之外的每次強制重連都會把
   `orders_since_event` / `last_event_at` 歸零，下一階必須等市場動到觸發 requote 才有判死依據。

   2026-08-16 實測（引擎 `09:29:08` 重啟，跑 `f4bdd8a`，證據為 `log/as_terminal_max.log`）：

   | 時點 | 事件 | 距上一階 |
   |---|---|---|
   | 09:29:08 | 已訂閱 userData stream（t0） | — |
   | 09:39:08 | 判死 + 強制重連 1/3（`8 張單無推送、靜默 605s`） | t0+10 分 |
   | 10:04:08 | 重連 2/3（`靜默 1500s`） | +25 分 |
   | 10:21:08 | 重連 3/3（`靜默 1020s`，`next_attempt_at = 11:06:08`） | +17 分 |
   | 11:06:08 | 退避到期但**沒**進 `given_up`——10:21 後零 requote ⇒ `orders_since_event=0` | — |
   | 11:27:08 | 進 `given_up` + 第二封 Telegram | +66 分（其中 21 分純粹在等 requote） |

   ⇒ **全程 118 分鐘**，之後零重連、每 3600s 節流提醒一次，符合 §5.2。
   當日 requote 間隔實測 **25~80 分鐘**（03:50 / 05:12 / 06:13 / 07:13 / 07:38 / 09:29 /
   10:03 / 10:20 / 11:26）⇒ **安靜市況下 `given_up` 的抵達時間無上界**（極端情形即 §8.2-1
   的裝死模式：永遠停在 `degraded`）。驗收與告警文案都不得承諾任何固定分鐘數。

   兩封 Telegram 皆經使用者確認實際收到（09:39 的 ⚠️、11:27 的 ⛔），log 零 `Telegram 發送失敗`
   ⇒ 端到端告警路徑活體驗證完成。
