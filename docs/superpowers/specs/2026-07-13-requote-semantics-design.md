...
- **低端主 gate**：舊語意（factor=0.5）+ 現狀 seed 倉位，套在 07-12 14:51
  ...
  誠實標註）。
- **高端參照 gate**：新語意（掛到成交）tick sim 與 1m GridBacktester 在同一
  窗口、同參數下對比成交筆數。1m 已知偏樂觀（多層漏算 ~6.4%、掛單活滿整根
  bar），tick sim 嚴格穿越偏保守 → 預期 tick ≤ 1m。
  PASS：tick sim 成交數落在 1m 的 **[0.2×, 1.0×]** 區間——低於 0.2× 表示
  fill 引擎系統性吃單失敗（如穿越判定+延遲窗口交互 bug），高於 1.0× 表示
  模擬偷跑。區間為數量級守門，非精確校準，報告需附兩者逐窗口對照。
