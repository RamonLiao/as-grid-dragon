...
- **rename 機制本身不改任何行為**。改名、`from_dict` 相容、舊 key 清除三者對回測輸出零影響——已由跨新舊樹的 differential 驗證（同一份資料、同一路徑，result dict byte-identical）。

> ### ⚠️ 執行期修訂（2026-07-26）：「行為零變更」的宣稱範圍已限縮
> 原始 spec 寫「不改任何行為」。實際交付**有兩處刻意的行為變更**，都是正確的修正，但不得被「零變更」掩蓋：
> 1. **UI clamp `max_value` 15 → 125**（`web/pages/2`、`web/pages/3`）。舊值 15 小於生產值 20 ⇒ 打開頁3 或從頁2 編輯任何欄位送出，槓桿就被靜默降級成 15。這是既有 bug，經使用者裁決在本任務順手修掉。
> 2. **生產 config 值 20 → 5**（使用者裁決）。四個 symbol 的回測槓桿假設改為交易所實測值。
>    ⇒ **所有遷移前的回測結果不可再與遷移後的結果直接比較**——保證金與強平模型的輸入變了。
>
> 「零變更」僅適用於 rename 機制，不適用於整條 branch。
